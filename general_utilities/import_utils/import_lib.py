...
def download_bgen_file(chrom_bgen_index: BGENInformation) -> Tuple[Path, Path, Path, Path, Path]:
    ...
    # the BGEN file comes first
    bgen = chrom_bgen_index['bgen'].get_file_handle()
    # Next we do the sample
    bgen_sample = chrom_bgen_index['sample'].get_file_handle()

    # Then we need to add this little fix for the imputed data based on the sample file...
    # only do the following to the imputed data (we can tell by the smelly sample file):
    sample_df = pd.read_csv(bgen_sample, sep=' ')
    # if the first row is '0 0 0 0'
    if [str(x).strip() for x in sample_df.iloc[0, 0:4]] == ['0', '0', '0', '0']:
        print("Working with newer imputation data, going to fix sample and index files")
        # The sample file provided for the newer imputation has a formatting error. The column datatype identifier in
        # row 2 is '0 0 0 0' when it should be '0 0 0 D'. The 'D' causes plink2 to err, which causes lots of issues
        # with my pipeline. Fixing it here with a simple sed command.
        fix_cmd = f'sed -i \'s_0 0 0 0_0 0 0 D_\' {bgen_sample}'
        cmd_executor.run_cmd(fix_cmd)

        # TODO: REMOVE WHEN INDICES ARE FIXED
        cmd = f'bgenix -g {bgen} -index -clobber'
        cmd_executor.run_cmd_on_docker(cmd)
        bgen_index = Path(f'{bgen}.bgi')
        # END TO DO
    else:
