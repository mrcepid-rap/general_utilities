...
def ingest_tarballs(association_tarballs: Union[InputFileHandler, List[InputFileHandler]]) -> Tuple[TarballType, List[Path]]:
    ...
    for tar_file in tar_files:
        if tarfile.is_tarfile(tar_file):
            ...
            # This is to check if they have added some weird suffix
            if not Path(f'{tarball_prefix.absolute()}.tar.gz').exists():
                raise IOError(f'Provided association tarball ({tar_file}) '
                              f'does not have the expected .tar.gz suffix. '
                              f'Please ensure suffix is only .tar.gz.')

            tarball_prefixes.append(tarball_prefix)
            tar = tarfile.open(tar_file, 'r:gz')
            tar.extractall()

            # Now check what type of tarball this is
